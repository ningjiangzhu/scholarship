...
var context1 = canvas.getContext("2d");//舞台，getContext() 方法可返回一个对象，该对象提供了用于在画布上绘图的方法和属性。
var setCanvas = document.getElementById("setCanvas");//演员
var setContext = setCanvas.getContext("2d");//舞台，getContext() 方法可返回一个对象，该对象提供了用于在画布上绘图的方法和属性。

var num; //定义容器接收验证码
draw(context1);
canvas.onclick = function () {
    context1.clearRect(0, 0, 120, 34);//在给定的矩形内清除指定的像素
    draw(context1);
}
setCanvas.onclick = function () {
    setContext.clearRect(0, 0, 120, 34);//在给定的矩形内清除指定的像素
    draw(setContext);
}
// 随机颜色函数
function getColor() {
...
function checkLogin_id() {
    if(checkLoginId() && checkCheckCode()){
        var login_id = $("#login_id").val();
        $.ajax({
            type:"post",
            url:"/login/checkLogin_id",
            dataType:"text",
            data:{
                login_id:login_id
            },
            success:function (result) {
                if(result == "1"){
                    $("#errorLogin_id").html("用户不存在");
                }else if (result == "2"){
                    alert("您未绑定手机号，无法找回密码，密码为初始密码");
                }else{
                    $("#checkLogin_id").hide();
                    $("#checkPhone").show();
...
function checkSetCheckCode() {
    var setCheckCode = $("#setCheckCode").val();
    if(!setCheckCode){
        $("#errorSetCheckCode").html("请填写验证码");
        return false;
    }else if(num != setCheckCode) {
        $("#errorSetCheckCode").html("验证码错误");
        return false;
    }else{
        $("#errorSetCheckCode").html("");
        return true;
    }
}

function setPassword() {
    if(checkPassword() && checkCheckPassword() && checkSetCheckCode()){
        var password = $("#password").val();
        $.ajax({
            type:"post",
